...
class Mind_Accounts_Block_Adminhtml_Accounts_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
  function getcodes()
  {
    $codeq = mysql_query("SELECT * FROM tbl_code as a JOIN tbl_rate as b  WHERE a.activated='1' AND b.rate_id=a.code_package");
    while($sss = mysql_fetch_array($codeq))
    {
    $counterx = mysql_num_rows(mysql_query("SELECT * FROM tbl_accounts WHERE code_id='".$sss['code_value']."'"));
    if($counterx==0)
    {
...
if($this->getRequest()->getParam('id')!='')
{
    ...
    'label' => Mage::helper('accounts')->__('Code_id'), 
    'class' => 'required-entry', 
    'required' => true, 
    'name' => 'code_id', 
    'disabled' => disabled, 
    ...
    'label' => Mage::helper('accounts')->__('Balance'), 
    'class' => 'required-entry', 
    'disabled' => disabled, 
    'name' => 'balance', 
    ));
...
{
    $fieldset->addField('code_id', 'select', array(
    'label'     => Mage::helper('accounts')->__('Select'),
    'class'     => 'required-entry',
    'required'  => true,
    'name'      => 'code_id',
    'onchange' => "",
    'values' => $this->getcodes(),
    ));
}

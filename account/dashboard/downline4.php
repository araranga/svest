...
#print_r($_GET['parent']);
$satchbaby = array();
foreach($_GET['parent'] as $varvarx)
{
	$satchbaby[] = $varvarx[0];
}
asort($satchbaby);
foreach($satchbaby as $var)
{	
	$user_id_table = '';
	breakfree_child(array($var),"parent_test_".$var);
	$userpaid = getfuckinguser($_GET["parent_test_".$var]);
	$userid = $var;
	#echo $userid." ";
	if($userpaid<15)
	{	
...
$user_id_table =  $_SESSION['accounts_id'];
